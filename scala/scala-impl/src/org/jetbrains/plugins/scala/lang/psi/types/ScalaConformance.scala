...
trait ScalaConformance extends api.Conformance with TypeVariableUnification {
  ...
  private class LeftConformanceVisitor(key: Key, visited: Set[PsiClass]) extends ScalaTypeVisitor {
    ...
    override def visitCompoundType(c: ScCompoundType) {
      ...
      Un                            === compn*/
      def workWithSignature(s: TermSignature): Boolean = {
        val processor = new CompoundTypeCheckSignatureProcessor(s,  constraints, s.substitutor)
      ...
      val isSuccess = c.components.forall(comp => {
        ...
        case (_, sig) => workWithSignature(sig)
      } && c.typesMap.forall {
